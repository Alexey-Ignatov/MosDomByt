...
class OrderFinalPresenter(
    ...
    override val TAG: String = "ru.acurresearch.dombyta.ui.ORDER_FINAL"
            ...
            it.flatMap { state }
                ...
                .switchMapSingle { state ->
                    orderInteractor.clearCurrentOrder()
                        .andThen(orderInteractor.saveCurrentOrder(state.currentOrder.content!!, orderAlreadyExists))
                        .map { state.copy(
                            currentOrder = BaseLCE(false, it, null)
                        ) }
